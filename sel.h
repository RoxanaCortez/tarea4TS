void createLocalA(Matrix &A,mesh m){ //matriz local A = t/8 [-1 1]
    float t = m.getParameter(const_t);                    //[-1 1]
    A.at(0).at(0) += -t/8;  A.at(0).at(1) += t/8;
    A.at(1).at(0) += -t/8;  A.at(1).at(1) += t/8;
}

void createLocalB(Matrix &B,mesh m){ //matriz local B = k/l [1 -1]
    float l = m.getParameter(ELEMENT_LENGTH);             //[-1 1]
    float k = m.getParameter(const_k);
    B.at(0).at(0) += k/l;      B.at(0).at(1) += -k/l;
    B.at(1).at(0) += -k/l;     B.at(1).at(1) += k/l;
}

void createLocalC(Matrix &C,mesh m){ //matriz local C =lambda/3 [-1 1]
    float lambda = m.getParameter(const_lambda);            //[-1 1]
    C.at(0).at(0) += -lambda/3;    C.at(0).at(1) += lambda/3;
    C.at(1).at(0) += -lambda/3;    C.at(1).at(1) += lambda/3;
}

void createLocalX(Matrix &X,mesh m){ //matriz x = v/l [1 -1]
    float l = m.getParameter(ELEMENT_LENGTH);       //[-1 1]
    float v = m.getParameter(const_v);
    X.at(0).at(0) += v/l;      X.at(0).at(1) += -v/l;
    X.at(1).at(0) += -v/l;     X.at(1).at(1) += v/l;
}

void createLocalD(Matrix &D,mesh m){//matriz D=3a/2 [-1 1]
    float a = m.getParameter(const_a);             //[-1 1]
    D.at(0).at(0) += -(3*a)/2;  D.at(0).at(1) += (3*a)/2;
    D.at(1).at(0) += -(3*a)/2;  D.at(1).at(1) += (3*a)/2;
}

void createLocalE(Matrix &E,mesh m){//matriz local E= d/2 [-1 1]
    float d = m.getParameter(const_d);                  //[-1 1]
    E.at(0).at(0) += -d/2;  E.at(0).at(1) += d/2;
    E.at(1).at(0) += -d/2;  E.at(1).at(1) += d/2;
}

Matrix createLocalK(int element,mesh &m){
    Matrix K,A,B,C,X,D,E;

    zeroes(A,2);
    zeroes(B,2);
    zeroes(C,2);
    zeroes(X,2);//matriz X 2x2
    zeroes(D,2);
    zeroes(E,2);// matriz E 2x2
    createLocalA(A,m);
    createLocalB(B,m);
    createLocalC(C,m);
    createLocalX(X,m);//creacion de la matriz X local
    createLocalD(D,m);
    createLocalE(E,m);//creacion de la matriz E local

    Vector row1, row2, row3, row4;

    //sistema matricial [(A+B) (C+X)]
    //                  [  D     E  ]
    row1.push_back(A.at(0).at(0)+B.at(0).at(0));
    row1.push_back(A.at(0).at(1)+B.at(0).at(1));
    row1.push_back(C.at(0).at(0)+X.at(0).at(0));//
    row1.push_back(C.at(0).at(1)+X.at(0).at(1));

    row2.push_back(A.at(1).at(0)+B.at(1).at(0));
    row2.push_back(A.at(1).at(1)+B.at(1).at(1));
    row2.push_back(C.at(1).at(0)+X.at(1).at(0));
    row2.push_back(C.at(1).at(1)+X.at(1).at(1));
...
Vector createLocalb(int element,mesh &m){
    ...
    float y = m.getParameter(const_y);
    float n = m.getParameter(const_n);
    float l = m.getParameter(ELEMENT_LENGTH);

    b.push_back(y*l/2);//sistema matricial b
    b.push_back(y*l/2);
    b.push_back(n*l/2);
    b.push_back(n*l/2);
